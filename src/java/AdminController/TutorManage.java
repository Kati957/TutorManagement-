...
public class TutorManage extends HttpServlet {

    private static final Logger LOGGER = Logger.getLogger(TutorManage.class.getName());
    private static final String TUTOR_EDIT_JSP = "/admin/tutor-edit.jsp";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (edit != null) {
                ...
                if (editUser != null && editUser.getRoleID() == 3) {
                    request.setAttribute("editUser", editUser);
                } else {
                    request.setAttribute("error", "Không tìm thấy tutor để chỉnh sửa!");
                }
            } else {
                request.setAttribute("error", "Không có tutor được chọn để chỉnh sửa!");
            }
            request.getRequestDispatcher(TUTOR_EDIT_JSP).forward(request, response);
        } catch (Exception ex) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (userID == null || userID.trim().isEmpty()) {
                request.setAttribute("error", "User ID không được để trống!");
                request.getRequestDispatcher(TUTOR_EDIT_JSP).forward(request, response);
                return;
            }
            ...
            if (dao.updateUser(user)) {
                session.setAttribute("success", "Cập nhật tutor thành công!");
                response.sendRedirect(request.getContextPath() + "/admin/TutorList");
            } else {
